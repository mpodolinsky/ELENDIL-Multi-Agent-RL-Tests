...
# W&B Setup
# -----------------------
def train(ippo_config: yaml, hemac_config: yaml, project_name: str = None, run_name: str = None):
    if project_name is None or run_name is None:
        raise ValueError("Please provide both project_name and run_name for W&B logging.")

    with open("ippo_default_config_HeMAC.yaml", "r") as f:
        config = yaml.safe_load(f)

    wandb.init(
        project=project_name,
        name=f"{run_name}_{time.strftime('%Y%m%d-%H%M%S')}",
        config=config,
        monitor_gym=True,
    )
    ...
    def make_env(hemac_config=hemac_config):
    ...
    while total_steps < max_steps:
        ...
        while True:
            ...
            SAVE_INTERVAL = 25  # save a checkpoint every 25 PPO updates
            ...
            if total_steps % (rollout_steps * NUM_ENVS) == 0 or total_steps >= max_steps:
                ...
                if updates_completed % SAVE_INTERVAL == 0 or total_steps >= max_steps:
                    ...
                    artifact = wandb.Artifact("ippo-agent-1q1o", type="model")
...
if __name__ == "__main__":
    train("ippo_default_config_HeMAC.yaml", "hemac_env_config.yaml")